...
The simplest choice: $\boldsymbol{\Pi}_0 = \mathbf{I}_n$, assuming independent unit-variance priors. This carries no domain-specific information, but the direct precision formulation still produces a valid precision matrix---unlike inverting a shrunk covariance, which can amplify estimation error.
...
\end{equation}
where $\sigma_i^2$ are the diagonal entries of $\mathbf{S}$. The factor of 2 mirrors the $2/N_\ell$ scaling of the cosmological target~\eqref{eq:cosmo_target}. This variant tests how much of the direct precision shrinkage performance depends on domain-specific target knowledge versus the general framework itself.
