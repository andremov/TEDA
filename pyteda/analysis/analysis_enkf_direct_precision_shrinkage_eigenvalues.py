...
class AnalysisEnKFDirectPrecisionShrinkageEigenvalues(Analysis):
    ...
    def __init__(self, model, **kwargs):
        """
        Initialize an instance of AnalysisEnKFDirectPrecisionShrinkageEigenvalues.
    ...
    def compute_target_precision_matrix(self, S_inv):
        ...
        """
        # Compute the eigenvalues of S_inv
        eigenvalues, _ = np.linalg.eigh(S_inv)  # eigh is used for symmetric matrices

        # Sort eigenvalues in ascending order
        sorted_eigenvalues = np.sort(eigenvalues)

        # Construct the diagonal matrix Pi0^(1)
        Pi0 = np.diag(sorted_eigenvalues)
    ...
    def compute_shrinkage_weights(self, S_inv, Pi0, data_vector_length, ensemble_size):
        ...
        """
        # Compute squared trace norm of S_inv (sum of singular values, squared)
        singular_values = np.linalg.svd(S_inv, compute_uv=False)
        squared_trace_norm = np.sum(singular_values) ** 2

        # Compute squared Frobenius norm of S_inv (sum of squared singular values)
        frobenius_norm_sq_S_inv = np.sum(singular_values ** 2)
        ...
        # Compute α* and β* per Bodnar et al. (2016) Eq. 4.4-4.5
        # Numerator uses trace norm; denominator uses Frobenius norm of S_inv
        numerator_alpha = (ensemble_size ** -1) * squared_trace_norm * frobenius_norm_sq_Pi0
        denominator_alpha = (frobenius_norm_sq_S_inv * frobenius_norm_sq_Pi0) - (trace_S_inv_Pi0 ** 2)
    ...
    def get_precision_matrix(self, DX):
        ...
        # Use raw S^{-1} in the Bodnar shrinkage formula per Looijmans et al. (2024) Eq. 10-11.
        # The Hartlap factor γ = (n-d-2)/(n-1) is NOT applied here because the Bodnar
        # formula was derived for raw S^{-1} and already accounts for bias via the (1-d/n) term.
        S_inv = Pi_S_unscaled
