...
Figure~\ref{fig:regime_map} places both experiments on a two-dimensional plane spanned by the concentration ratio $n/N_e$ and the structural distance from the shrinkage target. The map summarises which regime each combination of method and target falls into, and which method choice is appropriate.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/shrinkage_regime_map.pdf}
\caption{Regime map for shrinkage-based EnKF precision estimation. Horizontal axis: concentration ratio $n/N_e$. Vertical axis: structural distance from the shrinkage target (e.g.\ off-diagonal energy of the true precision relative to a diagonal target). The two test domains of this work occupy opposite quadrants: BOSS cosmological data is near-diagonal (low vertical), Lorenz~96 is dense (high vertical). The recommended method changes across the map; see text.}
