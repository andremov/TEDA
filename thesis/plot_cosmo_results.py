# -*- coding: utf-8 -*-
"""
Plot cosmological results from individual method JSON files.

Run AFTER all run_cosmo_method.py jobs finish:
    python thesis/plot_cosmo_results.py

Reads from: paper/figures/cosmo_traces/*.json
Outputs:    paper/figures/cosmo_rmse_time_ne{24,30,40,50}.pdf
            paper/figures/cosmo_rmse_boxplot.pdf
"""
import os, json
import numpy as np
import matplotlib
...
TRACE_DIR = os.path.join(FIG, 'cosmo_traces')
TRANSIENT = 50

METHOD_NAMES = {
    'EnKF': 'EnKF', 'Cholesky': 'EnKF-Cholesky',
    'Identity': 'Identity Shrinkage', 'Scaled': 'Scaled Shrinkage',
    'Eigenvalue': 'Eigenvalue Shrinkage', 'Cosmo': 'Cosmological Shrinkage',
    'LedoitWolf': 'Ledoit-Wolf',
}
COLORS = {
...
if __name__ == '__main__':
    data = load_all()
    if not data:
        print("No data found. Run run_cosmo_method.py first.")
        exit(1)

    # RMSE time series (first run only) at each Ne
